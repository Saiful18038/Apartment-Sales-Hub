...
function TeamDetailContent() {
  ...
  // Period carries over from whatever was selected on the Reports page
  // (passed in the link), and can be changed here too.
  ...
  const { data: teamData, loading, error } = useApi(`/reports/team-summary?year=${year}&month=${month}`);
    ...
    <div className="space-y-4">
      <div>
        ...
        <h2 className="text-lg font-semibold text-slate-800">
          Individual Team Performance{team ? ` — ${team.team}` : ""}
        </h2>
      </div>
      ...
      {loading ? (
        ...
        <ErrorBanner message="Team not found." />
      ) : (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          <div className="text-sm text-slate-500 mb-3">
            Team Leader: <span className="font-medium text-slate-800">{team.leader || "—"}</span>
            <span className="text-slate-400"> · {periodLabel(year, month)}</span>
          </div>
