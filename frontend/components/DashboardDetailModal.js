...
import { Modal, Th, Td, StatusPill } from "@/components/ui";
...
/**
 * Owner's request: every stat card on the Dashboard opens a details view "in
 * its place" (an in-page modal, not a navigation away) listing the rows
 * behind that number. Built from the same already-loaded, already
 ...
 * withheld per FlatResource::canView, so an Employee clicking a card sees
 * exactly the same privacy boundary as everywhere else in the app, not a
 * bypass of it.
 */
export default function DashboardDetailModal({ detailKey, onClose, data }) {
  ...
  return (
    <Modal title={TITLES[detailKey]} onClose={onClose} wide>
      {detailKey === "zones" && (
